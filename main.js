...
function setupBoardSize() {
  ...
  if (tmp_width > 30 || tmp_height > 30) {
    alert('Input Dimension is Too Large! At Most 30.');
    return;
  }
...
function initialize() {
  ...
  $('#start_search')[0].onclick = function (event) {
    if (graph.source == null || graph.target == null) {
      ...
      search = new Search(graph, "A*");  //$("input[name='search_algo']:checked").val()
...
var button = document.getElementById('btn_code'); 

button.onclick = function() {
    var div = document.getElementById('pseudocode');
